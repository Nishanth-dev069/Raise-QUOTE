...
import { Search, Calendar, Download, Menu, X, Plus, Package, LogOut, FileText, ChevronDown, Pencil } from "lucide-react"
  ...
  return (
    <div className="flex min-h-screen bg-[#FDFDFD]">
      ...
      <aside className={`fixed inset-y-0 left-0 z-50 flex w-64 flex-col border-r border-gray-100 bg-white transition-transform duration-200 lg:translate-x-0 ${isSidebarOpen ? "translate-x-0" : "-translate-x-full"}`}>
        <div className="flex h-16 items-center justify-between px-6 border-b border-gray-50">
          <div className="flex items-center gap-3">
            ...
            <div>
              <span className="text-xs font-black tracking-tighter text-black">RAISE LABS</span>
              <span className="block text-[8px] font-bold tracking-widest text-gray-400 uppercase">Equipment</span>
            </div>
          </div>
        ...
        <nav className="flex-1 space-y-1 p-4">
          ...
          <Link
            href="/quotations"
            className="flex items-center gap-3 rounded-xl bg-black px-4 py-3 text-sm font-bold text-white shadow-lg shadow-black/10 transition-all"
          >
        ...
        <div className="border-t border-gray-50 p-4">
          <div className="flex items-center gap-3 px-2 py-3 rounded-xl bg-gray-50/50">
            ...
            <div className="flex-1 overflow-hidden">
              <p className="truncate text-xs font-bold text-black">{user.full_name}</p>
              <p className="truncate text-[10px] font-medium text-gray-400 uppercase tracking-wider">{user.role}</p>
            </div>
      ...
      <main className="flex-1 transition-all lg:pl-64">
        ...
        <div className="mx-auto max-w-7xl px-4 py-8 lg:px-10 lg:py-10 space-y-8">
          <div>
                      ...
                      <TableRow key={q.id} className="hover:bg-gray-50/50 transition-colors border-gray-50">
                        ...
                        <TableCell className="px-6 py-4 text-right">
                          <div className="flex items-center justify-end gap-2">
                            <Link
                              href={`/?edit=${q.id}`}
                              className="inline-flex items-center gap-1.5 rounded-xl border border-gray-200 bg-white px-3 py-2 text-xs font-bold text-black shadow-sm transition-all hover:bg-gray-50 hover:border-black/20"
                            >
                            ...
                            {q.pdf_url && (
                              <a
                                ...
                                className="inline-flex items-center gap-1.5 rounded-xl bg-black px-3 py-2 text-xs font-bold text-white shadow-sm transition-all hover:bg-black/90"
                              >
